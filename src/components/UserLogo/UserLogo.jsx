...
const UserLogo = ({ userName, avatarUrl }) => {
  ...
  const renderUserContent = () => {
    if (avatarUrl) {
      // Если есть аватар, отображаем имя пользователя и аватар
      return (
        <>
          <h2>{userName}</h2>
      ...
      // Если нет аватара, но есть имя пользователя, отображаем имя и первую букву аватара
      const avatarInitial = userName.charAt(0).toUpperCase();
      return (
        <>
          <h2>{userName}</h2>
          <HeaderImage src={`https://via.placeholder.com/30?text=${avatarInitial}`} />
        </>
      );
    } else {
      // Если отсутствуют и имя, и аватар, отображаем букву 'V'
      return <h2>V</h2>;
    }
  };
